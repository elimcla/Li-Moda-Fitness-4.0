...
const CheckoutModal: React.FC<CheckoutModalProps> = ({ items, user, onClose, onSuccess }) => {
  ...
  const handleFinalizeOrder = async () => {
    ...
    try {
      ...
      // 1. Processamento de estoque e cupom no Firebase
      ...
      // 2. Redirecionamento unificado para WhatsApp (Independente de Pix ou Cartão)
      ...
      const whatsappMessage = `*PEDIDO REALIZADO - LI MODA FITNESS*%0A%0A*Cliente:* ${form.name}%0A*WhatsApp:* ${form.phone}%0A%0A*ITENS:*%0A${itemsList}${addressString}%0A%0A*Subtotal:* R$ ${subtotal.toFixed(2)}%0A*Desconto:* R$ ${discountValue.toFixed(2)}%0A*Frete:* R$ ${shippingValue.toFixed(2)}%0A*TOTAL:* R$ ${finalTotal.toFixed(2)}%0A%0A*Forma de Pagamento:* ${paymentMethod.toUpperCase()}%0A%0A_Estou enviando este comprovante para confirmação._`;
          ...
          <div className="space-y-6">
             <div className="flex gap-4">
              ...
              <label className="flex items-center gap-3 cursor-pointer">
                ...
                <span className="text-[10px] text-white/60 font-bold uppercase tracking-tight">Confirmo que enviarei o comprovante pelo WhatsApp</span>
              </label>
              
              <button onClick={handleFinalizeOrder} disabled={loading || !acceptTerms} className="w-full bg-[#CCFF00] text-black py-5 rounded-xl font-black uppercase tracking-widest shadow-xl shadow-[#CCFF00]/10 active:scale-95 transition-all">
