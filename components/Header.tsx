...
const Header: React.FC<HeaderProps> = ({ 
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 glass h-20 px-4 flex items-center justify-between border-b border-white/5">
      <div className="flex items-center gap-1 cursor-pointer group shrink-0" onClick={() => onNavigate('home')}>
        ...
        <span className="text-sm md:text-xl font-sora font-bold">Moda Fitness</span>
      </div>

      <nav className="hidden lg:flex gap-8 items-center text-sm font-medium tracking-wide">
        <button onClick={() => onNavigate('home')} className="hover:text-[#CCFF00] transition-colors font-bold uppercase tracking-widest text-[10px]">HOME</button>
        <button onClick={() => onNavigate('shop')} className="hover:text-[#CCFF00] transition-colors font-bold uppercase tracking-widest text-[10px]">LOJA</button>
        {user?.isAdmin && (
      ...
      <div className="flex items-center gap-2 md:gap-5">
        ...
        {user && !user.isAdmin && userData && (
          <div className="hidden md:flex flex-col items-end gap-1 mr-2">
            ...
            {loyalty.next && (
              <span className="text-[7px] text-white/30 font-bold uppercase">Faltam R$ {(loyalty.target - (userData.totalSpent || 0)).toFixed(2)} para {loyalty.next}</span>
            )}
          </div>
        )}

        <button onClick={() => onNavigate('shop')} className="relative p-2 text-white/40 hover:text-[#CCFF00] transition-colors">
          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"/></svg>
          {hasPromo && <span className="absolute top-2 right-2 w-2 h-2 bg-red-500 rounded-full animate-ping" />}
        </button>

        {user && !user.isAdmin && (
          <button onClick={markCouponRead} className={`relative p-2 transition-colors ${userData?.activeCoupon ? 'text-[#CCFF00]' : 'text-white/20'}`}>
        ...
        {user ? (
          <div className="flex items-center gap-2">
            <div className="flex flex-col items-end min-w-[70px]">
              ...
              <button onClick={onLogout} className="text-[8px] md:text-[9px] text-white/40 hover:text-red-500 uppercase font-bold">[ SAIR ]</button>
            </div>
            <div 
          ...
          <button onClick={() => onNavigate('login')} className="flex items-center gap-2 text-[10px] md:text-sm hover:text-[#CCFF00] transition-colors group px-2">
        ...
        <button onClick={onOpenCart} className="relative group p-2 rounded-full hover:bg-white/5 transition-colors">
          ...
          {cartCount > 0 && <span className="absolute -top-1 -right-1 bg-[#CCFF00] text-black text-[10px] font-bold w-5 h-5 rounded-full flex items-center justify-center">{cartCount}</span>}
        </button>
      </div>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-black/90 backdrop-blur-xl">
          <div className="w-full max-w-sm glass rounded-[2.5rem] p-8 border border-[#CCFF00]/30 text-center space-y-6 shadow-[0_0_100px_rgba(204,255,0,0.1)]">
            ...
            <div className="space-y-2">
              <h2 className="text-2xl font-sora font-black text-[#CCFF00] uppercase tracking-tighter">RECOMPENSA LIBODA!</h2>
            ...
            <button onClick={() => setShowCouponModal(false)} className="w-full py-4 bg-white/5 hover:bg-white/10 rounded-xl text-[10px] font-black uppercase tracking-widest text-white/40 transition-all">Fechar Presente</button>
          </div>
        </div>
      )}
    </header>
